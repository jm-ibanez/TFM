...
class Galaxy_Dataset(SegmentationDataset):

    num_classes = 2
    label_names = ['galaxy','background'] 


    color_map   = np.array([
        [255, 255, 128], # galaxy
        [ 0,  0, 50] # sky background
    ...
    def __init__(self, root, subset='train', transform=None, file_path=False, num_images=None, mode='labeled'):
        ...
        self.mode = mode
        super().__init__(root, subset,  
                img_path = 'images', label_path='annotations', pattern='*',
                img_suffix = '.jpg' , label_suffix='_mask.png', transform=transform, 
                    file_path=file_path, num_images=num_images)
...
if __name__ == "__main__":
    ...
    # cs = CityscapesDataset('/opt/TFM/DATASETS/cityscapes')
    # show_data(cs)
    ...
    # mv = IDD_Dataset('/opt/TFM/DATASETS/idd20k_lite')
    # show_data(mv)
    
    gal = Galaxy_Dataset('/opt/TFM/DATASETS/tutorial_usss', transform=None)
    print("LEN=", len(gal))
    show_data(gal)