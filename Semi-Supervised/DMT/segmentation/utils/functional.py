...
def resize(img, size, interpolation=Image.BILINEAR):
    ...
    _PIL_TO_TORCH_INTERP_MODE = {
        ...
        Image.LINEAR: "bilinear",  # It seems this should be what people want
    }
    unsqueeze_one = False
    unsqueeze_two = False
    if img.dim() == 3:
        ...
        img = img[None][None]
        unsqueeze_two = True
    if _PIL_TO_TORCH_INTERP_MODE[interpolation] == 'nearest':
        if img.dtype == torch.int64:
            out = torch.nn.functional.interpolate(img.float(), size=size,
                                                  mode=_PIL_TO_TORCH_INTERP_MODE[interpolation]).long()
        else:
...
def pad(img, padding, fill=0, padding_mode='constant'):
    ...
    if isinstance(img, torch.Tensor):
        l, t, r, b = padding
        return torch.nn.functional.pad(input=img, pad=(l, r, t, b), mode=padding_mode, value=fill)
