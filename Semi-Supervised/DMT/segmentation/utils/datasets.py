...
class StandardSegmentationDataset(torchvision.datasets.VisionDataset):
    def __init__(self, root, image_set, transforms=None, transform=None, target_transform=None, label_state=0,
        ...
        if data_set == 'voc':
            ...
            self._galax_init(root, image_set, label_state)
        else:
            raise ValueError
    ...
    def __getitem__(self, index):
        ...
        # print("READING FILE", self.images[index])
        if self.has_label:
    ...
    def _galax_init(self, root, image_set, label_state):
        # It's tricky, lots of folders
        image_dir = os.path.join(root, 'images')
        if label_state == 0:
            mask_dir = os.path.join(root, 'annotations')
        else:
            mask_dir = os.path.join(root, 'annotationsPseudo')

        if image_set == 'val' or image_set == 'test':
            image_dir = os.path.join(image_dir, image_set)
            mask_dir = os.path.join(mask_dir, image_set)
        elif image_set == 'valtiny':
            image_dir = os.path.join(image_dir, 'valtiny')
            mask_dir = os.path.join(mask_dir, 'valtiny')
        else:
            image_dir = os.path.join(image_dir, 'train')
            mask_dir = os.path.join(mask_dir, 'train')
        ...
        self.images = [os.path.join(image_dir, x + ".jpg") for x in file_names]
        self.masks = [os.path.join(mask_dir, x + "_mask" + self.mask_type) for x in file_names]
...
class SegmentationLabelsDataset(torch.utils.data.Dataset):
    def __init__(self, root, image_set, data_set='voc', mask_type='.png'):
        if data_set == 'voc':
            ...
            pseudo_dir = os.path.join(root, 'annotationsPseudo/train')
            target_dir = os.path.join(root, 'annotations/train')
            list_dir = os.path.join(root, 'data_lists')
        else:
            raise ValueError
