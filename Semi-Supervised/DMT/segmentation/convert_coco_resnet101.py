...
for key in coco:
    ...
    # galax
    if my_galax[my_key].shape == coco[key].shape:
        galax_shape_match += 1
        my_galax[my_key] = coco[key]
    else:
        galax_shape_not_match += 1
...
print(str(galax_shape_match) + ' galaxy shapes matched!')
print(str(galax_shape_not_match) + ' galaxy shapes are not a match.')

print('Saving models...')

voc_net.load_state_dict(my_voc)
city_net.load_state_dict(my_city)
galax_net.load_state_dict(my_galax)
...
save_checkpoint(net=galax_net, optimizer=None, lr_scheduler=None, is_mixed_precision=False,
                filename='galax_coco_resnet101.pt')
print('Complete.')
