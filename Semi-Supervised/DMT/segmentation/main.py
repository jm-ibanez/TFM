...
# For galaxy
from utils.common import colors_galax, categories_galax, sizes_galax, num_classes_galax, base_galax
...
def colormap_galaxy(n):
    cmap = np.zeros([n, 3]).astype(np.uint8)
    cmap[0,:] = np.array([0, 255, 128])  # background
    cmap[1,:] = np.array([255, 0, 0])  # galaxy
...
def init(batch_size_labeled, batch_size_pseudo, state, split, input_sizes, sets_id, std, mean,
    ...
    elif data_set == 'galax':  # All the same size (whole set is down-sampled by 2)
        base = base_galax
        workers = 8
        transform_train = Compose(
            [ToTensor(keep_scale=keep_scale, reverse_channels=reverse_channels),
    ...
    # Testing
    if state == 3:
        return val_loader, test_set.images
    else:
...
def test_one_set_and_save(loader, device, net, categories, num_classes, output_size, is_mixed_precision, file_name_lists):
    # Evaluate on 1 data_loader (cudnn impact < 0.003%)
    net.eval()
    conf_mat = ConfusionMatrix(num_classes)
    with torch.no_grad():
        ...
        for image, target in tqdm(loader):
            ...
            with autocast(is_mixed_precision):
                ...
                # now, save
                filenameSave = "./predicions/" + os.path.basename(file_name_lists[i]).split('.')[0] + ".jpg"
                print("Image %00d - %s --> %s"%(i, file_name_lists[i], filenameSave))
                #print("SHAPE_NEW_OUT", output.shape)
                label = output[0].argmax(0).data.byte()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='galax',
                        help='Train/Evaluate on PASCAL VOC 2012(voc)/Cityscapes(city/Galaxy) (default: galax)')
    ...
    if args.dataset == 'voc':
        ...
        colors = colors_city
    elif args.dataset == 'galax':
        num_classes = num_classes_galax
        input_sizes = sizes_galax
        categories = categories_galax
        colors = colors_galax
    else:
        raise ValueError
    ...
    # Testing
    if args.state == 3 or args.state == 4: # 4 = inference and save predicted image 
        net, optimizer = accelerator.prepare(net, optimizer)
        test_loader, file_name_lists  = init(batch_size_labeled=args.batch_size_labeled, batch_size_pseudo=args.batch_size_pseudo,
        ...
        if args.state == 3:
            ...
            # state == 4 --> inference and save image
            test_one_set_and_save(loader=test_loader, device=device, net=net, categories=categories, num_classes=num_classes,
                     output_size=input_sizes[2], is_mixed_precision=args.mixed_precision, file_name_lists=file_name_lists)
    else:
        x = 0
