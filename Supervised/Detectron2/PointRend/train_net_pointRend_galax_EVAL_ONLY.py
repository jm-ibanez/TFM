...
def setup(args):
    ...
    # NEWS !!! -- JM 4-Agosto-22
    cfg.MODEL.RESNETS.NORM = "BN"  # instead SyncBN (distributed computing)
    cfg.MODEL.SEM_SEG_HEAD.NORM = "BN" # instead SyncBN (distributed computing)
...
def run_my_inference(cfg, dataset_dir, custom_files=None):
    ...
    confidence_threshold = 0.9   # set a custom testing threshold
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = confidence_threshold   # set a custom testing threshold
    #cfg.MODEL.RETINANET.SCORE_THRESH_TEST = confidence_threshold
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = confidence_threshold
    ...
    for d in random.sample(dataset_dicts, len(dataset_dicts)):
        ...
        plt.imsave(os.path.join(os.path.join(cfg.OUTPUT_DIR, 'visualization/PointRend'), f + '.png'), img)
